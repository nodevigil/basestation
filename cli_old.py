...
class ScannerLibrary:
    ...
    def run_scan(self, target: str, org_id: str, scan_level: int = 1, 
                 protocol: str = None, debug: bool = False, 
                 enabled_scanners: List[str] = None, enabled_external_tools: List[str] = None) -> Dict[str, Any]:
        """
        Run scanning stage for a single target.
        
        Args:
            target: Target IP address or hostname to scan (required)
            org_id: Organization ID (required)
            scan_level: Scan intensity level (1-3)
            protocol: Protocol scanner to run (sui, filecoin)
...
def execute_command(config: Config, args) -> Dict[str, Any]:
    """
    Execute scanning command using the scanner library.
    ...
    # Always run scan stage - no other stages supported
    return _execute_scan_with_library(library, args)


def _execute_scan_with_library(library: ScannerLibrary, args) -> Dict[str, Any]:
    """Execute scan using the library."""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Target Scanning (org-id required)
  pgdn --target example.com --org-id myorg                    # Infrastructure scan of target
  pgdn --target example.com --org-id myorg --protocol sui     # Infrastructure + Sui protocol scan
  pgdn --target example.com --org-id myorg --scan-level 3     # Comprehensive target scan

  # Scanner Type Selection
  pgdn --target example.com --org-id myorg --type nmap           # Only run nmap scan
  pgdn --target example.com --org-id myorg --type geo            # Only run GeoIP lookup
  pgdn --target example.com --org-id myorg --type web            # Only run web analysis
  pgdn --target example.com --org-id myorg --type vulnerability  # Only run vulnerability scan
  pgdn --target example.com --org-id myorg --type ssl            # Only run SSL/TLS test
  pgdn --target example.com --org-id myorg --type docker         # Only check Docker exposure
  pgdn --target example.com --org-id myorg --type whatweb        # Only run web tech fingerprinting
  pgdn --target example.com --org-id myorg --type full           # Run all scanners (default)
  pgdn --target example.com --org-id myorg --debug --type nmap   # Debug nmap issues
  
  # Advanced scanner control (for developers)
  pgdn --target example.com --org-id myorg --scanners generic web
  pgdn --target example.com --org-id myorg --external-tools nmap whatweb
  pgdn --target example.com --org-id myorg --scanners geo --external-tools nmap
        """
    )
    ...
    parser.add_argument(
        '--org-id',
        required=True,
        help='Organization ID (required for all operations)'
    )
    ...
    parser.add_argument(
        '--target',
        required=True,
        help='Target IP address or hostname to scan (required)'
    )
    ...
    parser.add_argument(
        ...
        help='Run protocol-specific scanner (e.g., filecoin, sui)'
    )
